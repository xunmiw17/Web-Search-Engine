...
static int WriteHeader(FILE* f, int doctable_bytes, int memidx_bytes) {
  ...
  for (int i = 0; i < doctable_bytes + memidx_bytes; i++) {
    ...
    crc.FoldByteIntoCRC((uint8_t) ch);
  }
...
static int WriteHashTable(FILE* f, IndexFileOffset_t offset, HashTable* ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    ...
    int chain_len = LinkedList_NumElements(bucket);

    // Write the bucket record
    int bucket_rec_bytes =
          WriteHTBucketRecord(f, record_pos, chain_len, bucket_pos);
    if (bucket_rec_bytes == kFailedWrite) {
      return kFailedWrite;
    }
    record_pos += bucket_rec_bytes;

    // Write the bucket contents
...
static int WriteHTBucket(FILE* f, IndexFileOffset_t offset, LinkedList* li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    ElementPositionRecord elt_pos_rec(element_pos);
    elt_pos_rec.ToDiskFormat();
    if (fseek(f, record_pos, SEEK_SET) != 0) {
      return kFailedWrite;
    }
    if (fwrite(&elt_pos_rec, sizeof(ElementPositionRecord), 1, f) != 1) {
      return kFailedWrite;
    }
...
static int WriteDocIDToPositionListFn(FILE* f,
  ...
  IndexFileOffset_t cur_pos = offset;
  ...
  cur_pos += sizeof(DocIDElementHeader);
  ...
  return cur_pos - offset;  // you may want to change this
}
...
static int WriteWordToPostingsFn(FILE* f,
  ...
  if (fwrite(word, strlen(word), 1, f) != 1) {
    return kFailedWrite;
  }
  ...
  // STEP 19.
  // Calculate and return the total amount of data written.
  return sizeof(WordPostingsHeader) + strlen(word) +  ht_bytes;
}
}  // namespace hw3
